...
it('lets 2 users exchange stars', async () => {
    ...
    let tokenName2 = "TWO"
    let instance = await StarNotary.deployed();
    await instance.createStar(tokenName1, tokenId1, {from: address1})
    await instance.createStar(tokenName2, tokenId2, {from: address2})
    // 2. Call the exchangeStars functions implemented in the Smart Contract
    await instance.exchangeStars(tokenId1, tokenId2, {from: address1})
...
it('lets a user transfer a star', async () => {
    // 1. create a Star with different tokenId
    let tokenId = 103;
    let tokenName = "TRANSFER"
    let instance = await StarNotary.deployed();
    await instance.createStar(tokenName, tokenId, {from: accounts[1]})
